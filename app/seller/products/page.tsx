            ...
            {myProducts.map((product) => (
              ...
              >
                  ...
                  {product.tipe === 0 ? (
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-2">
                      {product.priceOptions?.map((opt) => (
                        <div key={opt.label} className="border border-gray-200 rounded-xl px-4 py-3 min-w-[120px]">
                          <p className="text-sm text-gray-500">{opt.label}</p>
                          <p className="text-lg font-bold text-gray-800">{formatPrice(opt.price)}</p>
                          <p className="text-xs text-gray-400 mt-1">Terjual 5</p>
                          <p className="text-xs text-gray-400">Total pendapatan {formatPrice(opt.price * 5)}</p>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex gap-2 mt-2 flex-wrap text-xs items-center">
                  <span style={{ backgroundColor: "#407BB5", color: "white" }} className="px-2 py-1 rounded font-bold">
                    Stok
                  </span>
                  <span style={{ backgroundColor: "#407bb5", color: "white" }} className="px-2 py-1 rounded">
                    {product.stock}{product.tipe === 0 ? product.unit : "unit"}
                  </span>

                  <span style={{ backgroundColor: "#5b99d7", color: "white" }} className="px-2 py-1 rounded font-bold">
                    Terjual
                  </span>
                  <span style={{ backgroundColor: "#5b99d7", color: "white" }} className="px-2 py-1 rounded">
                    5{product.tipe === 0 ? product.unit : "unit"}
                  </span>

                  <span style={{ backgroundColor: "#7db5ed", color: "white" }} className="px-2 py-1 rounded font-bold">
                    Total Pendapatan
                  </span>
                  <span style={{ backgroundColor: "#7db5ed", color: "white" }} className="px-2 py-1 rounded">
                    Rp75.000
                  </span>
                </div>
